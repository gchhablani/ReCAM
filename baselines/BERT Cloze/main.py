...
import random
import data_util
from data_util import ClozeSample
import numpy as np
import torch
import time
from pytorch_pretrained_bert.modeling import BertForCloze
...
def main():
    ...
    parser.add_argument(
        "--task_name", default="cloze", type=str, help="The name of the task to train."
    )
    parser.add_argument(
        "--output_dir",
        default="EXP/",
    ...
    parser.add_argument(
        "--cache_size", default=256, type=int, help="Total batch size for training."
    )
    parser.add_argument(
        ...
        for _ in range(int(args.num_train_epochs)):
            ...
            if args.do_eval and (
                ...
                for inp, tgt in valid_data.data_iter(shuffle=False):
                    ...
                    with torch.no_grad():
                        tmp_eval_loss, tmp_eval_accuracy, tmp_h_acc, tmp_m_acc = model(
                            inp, tgt
                        )
                    if n_gpu > 1:
                        tmp_eval_loss = (
