...
class Preprocessor(object):
    def __init__(self, args, device='cpu'):
        ...
        for sample in self.data:
            new_sample = self._create_sample(sample)
            if new_sample != None:
                high_cnt += sample['high']
                middle_cnt += (1 - sample['high'])
                self.data_objs += new_sample
            #break
...
class Loader(object):
    ...
    def _batchify(self, data_set, data_batch):
        ...
        options = torch.zeros(bsz, max_ops_num, 5, max_option_length).long()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='bert cloze')
    args = parser.parse_args()
    #'''
    data_collections = ['train', 'valid', 'test']
    for item in data_collections:    
        args.data_dir = './CLOZE/{}'.format(item)
