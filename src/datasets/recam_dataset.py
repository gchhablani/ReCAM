...
class ReCAMDataset(Dataset):
    ...
    def _preprocess(self, data):
        ...
        beg = max(0, answer_index - (self.config.question_truncate_length / 2))
            ...
            answer_index + (self.config.question_truncate_length / 2),
        )
        ...
        options_token_ids = [
            self.tokenizer.encode(data[f"option_{i}"].lower()) for i in range(5)
        ]
    ...
    def __getitem__(self, idx):
        ...
        return_dic["article_attention_mask"] = [1] * len(
            return_dic["article_token_ids"]
        )
        return_dic["question_attention_mask"] = [1] * len(
            return_dic["question_token_ids"]
        )
    ...
    def custom_collate_fn(self, batch):
        ...
        for sample, label in batch:
            ...
            max_options_len = max(
                max_options_len, max([len(i) for i in sample["options_token_ids"]])
            )
        ...
        # article_lengths = []  # [batch_size,]
        # question_lengths = []  # [batch_size,]
        # options_lengths = []  # [batch_size,5]

        for i in range(len(articles)):
            # article_lengths.append(len(articles[i]))
            articles[i] = articles[i] + [self.pad_id] * (
                max_article_len - len(articles[i])
            )
            # question_lengths.append(len(questions[i]))
            questions[i] = questions[i] + [self.pad_id] * (
                max_question_len - len(questions[i])
            )
            article_masks[i] = article_masks[i] + [self.pad_id] * (
                max_article_len - len(article_masks[i])
            )
            question_masks[i] = question_masks[i] + [self.pad_id] * (
                max_question_len - len(question_masks[i])
            )
            # options_lengths.append([])
            for option_index in range(len(options[i])):
                # options_lengths[-1].append(len(options[i][option_index]))
                options[i][option_index] = options[i][option_index] + [self.pad_id] * (
                    max_options_len - len(options[i][option_index])
                )
        ...
        return_dic = {
            ...
            # "article_lengths": torch.LongTensor(article_lengths),
            # "question_lengths": torch.LongTensor(question_lengths),
            # "options_lengths": torch.LongTensor(options_lengths),
        }
