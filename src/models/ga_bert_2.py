...
class GatedAttention(torch.nn.Module):
    def forward(self,attention_output, attention_mask=None, head_mask=None,encoder_hidden_states=None,encoder_attention_mask=None,past_key_value=None,output_attentions=False):
        question_att = encoder_hidden_states.permute(0,2,1)
        att_matrix = torch.bmm(attention_output,question_att)

        att_weights = torch.nn.functional.softmax(att_matrix.view(-1,att_matrix.size(-1)),dim=1).view_as(att_matrix)
        question_rep = torch.bmm(att_weights, encoder_hidden_states)
        question_to_article = torch.mul(attention_output, question_rep)
...
class GABertEncoder(torch.nn.Module):
    def __init__(self):
        ...
        bc.is_decoder = True
        bc.add_cross_attention = True

        self.m2 = BertModel(bc)

        for layer in self.m2.encoder.layer:
            layer.crossattention = GatedAttention()
    ...
    def forward(self, article_contexts, question_contexts, article_attention_mask=None, question_attention_mask=None):
        if question_attention_mask is not None:
            question_attention_mask = question_attention_mask.unsqueeze(1).unsqueeze(3)
        if article_attention_mask is not None:
            article_attention_mask = article_attention_mask.unsqueeze(1).unsqueeze(3)
        question_contexts = self.m1.encoder(question_contexts)[0]
        article_contexts =self.m2.encoder(article_contexts,article_attention_mask,encoder_hidden_states=question_contexts, encoder_attention_mask= question_attention_mask)[0] ##MASKS DON'T WORK. Just Testing.
...
@configmapper.map("models","gabertcloze2")
...
@configmapper.map("models","gabertqa2")
...
@configmapper.map("models","gabertclozeqa2")
