...
from transformers import BertPreTrainedModel, BertConfig, BertModel, AlbertModel
...
class MLPAttentionLogits(nn.Module):
    ...
    def forward(self, Q, K, V):
        ...
        scores = torch.matmul(K, Q.transpose(-1, -2))  # [batch_size, seq_len, dim]
        # scores = self.dropout(self.linear(M))  # [batch_size, seq_len, 1]

        return scores


@configmapper.map("models", "answeralbert")
class AnswerAttentionAlBert(nn.Module):
    def __init__(self, config):
        super(AnswerAttentionAlBert, self).__init__()
        self.config = config
        self.bert = AlbertModel.from_pretrained(self.config.albert_pretrained_name)
